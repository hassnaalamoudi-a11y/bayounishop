...
import { imagenPredict } from "./client.mjs";
import { fromImagenResponse, toImagenPredictRequest } from "./converters.mjs";
import {
...
const ImagenTryOnConfigSchema = z.object({
  sampleCount: z.number().int().min(1).optional().describe("Number of images to generate."),
  seed: z.number().int().optional().describe("Random seed for the image generation."),
  baseSteps: z.number().int().min(1).max(100).optional().describe("Number of denoising steps."),
  ...
  storageUri: z.string().optional().describe("Cloud Storage URI to store the generated images."),
