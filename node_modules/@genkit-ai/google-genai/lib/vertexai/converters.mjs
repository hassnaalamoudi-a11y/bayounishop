...
import { extractMediaArray } from "../common/utils.mjs";
import {
...
function toImagenPredictRequest(request) {
  const hasTryOnMedia = request.messages.some(
    (m) => m.content.some(
      (p) => p.metadata && (p.metadata.type === "personImage" || p.metadata.type === "productImage")
    )
  );
  if (hasTryOnMedia) {
...
function fromVeoOperation(fromOp, clientOpt) {
