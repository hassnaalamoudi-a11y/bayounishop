...
function toGeminiToolResponse(part) {
  if (part.toolResponse?.output === void 0 && !part.toolResponse?.content) {
    throw Error(
  ...
  const functionResponse = {
    ...
    ...part.toolResponse?.ref !== void 0 ? { id: part.toolResponse.ref } : {},
    response: {
      name: part.toolResponse.name,
      ...part.toolResponse?.output !== void 0 ? { content: part.toolResponse.output } : {}
    },
    ...part.toolResponse?.content !== void 0 ? { parts: part.toolResponse.content.map(toGeminiPart) } : {}
  };
  return maybeAddGeminiThoughtSignatureAndMetadata(part, {
    functionResponse
  });
}
function toGeminiResource(part) {
  if (!part.resource?.uri) {
    throw Error("Invalid ResourcePart: uri was missing.");
  }
  const media = {
    fileData: {
      mimeType: part.metadata?.mimeType || "application/octet-stream",
...
function toGeminiCustom(part) {
  ...
  if (part.custom?.geminiToolCall) {
    return maybeAddGeminiThoughtSignatureAndMetadata(part, {
      toolCall: part.custom.geminiToolCall
    });
  }
  if (part.custom?.geminiToolResponse) {
    return maybeAddGeminiThoughtSignatureAndMetadata(part, {
      toolResponse: part.custom.geminiToolResponse
    });
  }
...
function maybeAddGeminiThoughtSignatureAndMetadata(part, geminiPart) {
  ...
  if (part.metadata) {
    ...
    if ((0, import_types.isObject)(metadata) && Object.keys(metadata).length > 0) {
      updatedPart = {
        ...updatedPart,
...
function maybeAddThoughtSignatureAndMetadata(geminiPart, part) {
...
function fromGeminiFunctionResponse(part) {
  ...
  if ((0, import_types.isObject)(output) && "name" in output && "content" in output) {
...
function fromGeminiToolCall(part) {
  if (!part.toolCall) {
    throw new Error(
...
function fromGeminiToolResponse(part) {
  if (!part.toolResponse) {
    throw new Error(
...
function fromGeminiPart(part, previousChunks) {
  ...
  if (part.thoughtSignature) return fromGeminiThought(part);
...
const TEST_ONLY = { fromGeminiPart, toGeminiPart };
