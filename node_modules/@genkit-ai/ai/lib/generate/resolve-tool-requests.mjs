...
import { isPromptAction } from "../prompt.mjs";
import {
  ToolInterruptError,
  isToolRequest,
  resolveTools
...
async function resolveToolRequest(rawRequest, part, toolMap, middleware = [], runOptions) {
  ...
  const dispatch = async (index, req, ctx) => {
    if (index === middleware.length) {
      return executeTool(req, ctx);
    }
    const currentMiddleware = middleware[index];
    if (currentMiddleware.tool) {
      return currentMiddleware.tool(
        req,
        ctx,
        async (modifiedReq, opts) => dispatch(index + 1, modifiedReq || req, opts || ctx)
      );
    } else {
      return dispatch(index + 1, req, ctx);
    }
  };
  const executeTool = async (req, ctx) => {
    const output = await tool(req.toolRequest.input, ctx);
...
async function resolveToolRequests(rawRequest, generatedMessage, tools, middleware = []) {
...
async function resolveResumedToolRequest(rawRequest, part, toolMap, middleware = []) {
...
async function resolveResumeOption(registry, rawRequest, tools, middleware = []) {
...
async function resolveRestartedTools(registry, rawRequest, middleware = []) {
