...
import { resolveTools, toToolDefinition } from "../tool.mjs";
import { resolveMiddleware } from "./middleware.mjs";
import {
  assertValidToolNames,
  resolveResumeOption,
  resolveToolRequests
} from "./resolve-tool-requests.mjs";
...
async function generateActionImpl(registry, args) {
  const {
    rawRequest,
    middleware,
    currentTurn,
  ...
  const sharedPreviousChunks = [];
  const parser = format?.handler(rawRequest.output?.jsonSchema).parseChunk;
  if (middleware && middleware.length > 0) {
    const dispatchGenerate = async (index, request, currentTurn2, messageIndex2, ctx) => {
      ...
      if (currentMiddleware.generate) {
        const wrappedOnChunk = ctx.onChunk ? (c) => {
          if (c instanceof GenerateResponseChunk) {
            ctx.onChunk(c);
          } else {
            const chunk = new GenerateResponseChunk(c, {
              index: c.index !== void 0 ? c.index : messageIndex2,
              role: c.role !== void 0 ? c.role : "model",
              previousChunks: [...sharedPreviousChunks],
              parser
            });
            sharedPreviousChunks.push(c);
            ctx.onChunk(chunk);
          }
        } : void 0;
        return currentMiddleware.generate(
          { request, currentTurn: currentTurn2, messageIndex: messageIndex2 },
          { ...ctx, onChunk: wrappedOnChunk },
          async (modifiedEnvelope, opts) => dispatchGenerate(
            index + 1,
            modifiedEnvelope?.request || request,
            modifiedEnvelope?.currentTurn !== void 0 ? modifiedEnvelope.currentTurn : currentTurn2,
            modifiedEnvelope?.messageIndex !== void 0 ? modifiedEnvelope.messageIndex : messageIndex2,
            opts || ctx
          )
        );
      } else {
...
async function generateActionTurn(registry, {
  rawRequest,
  middleware,
  currentTurn,
  messageIndex,
  ...
  var response;
  const sendChunk = streamingCallback && ((chunk) => streamingCallback(makeChunk("model", chunk)));
  const dispatchModel = async (index, req, actionOpts) => {
    ...
    if (currentMiddleware.model) {
      return currentMiddleware.model(
        req,
        actionOpts,
        async (modifiedReq, opts) => dispatchModel(index + 1, modifiedReq || req, opts || actionOpts)
      );
    } else {
  ...
  const { revisedModelMessage, toolMessage, transferPreamble } = await resolveToolRequests(
    rawRequest,
    generatedMessage,
    tools,
    middleware || []
